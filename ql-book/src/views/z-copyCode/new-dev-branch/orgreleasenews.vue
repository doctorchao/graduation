<template>
  <div class="ssss_container">
    <!-- <van-nav-bar v-show="showTagListAndNav" :left-arrow= isHide v-bind:left-text="isHide?'z社区资讯(' + totalCount + ')':''" @click-left="isHide?pageBack():''" style="background-color: #F3F3F3;" v-if="isHide">
    </van-nav-bar> -->
    <div class="content">
      ...
      <div class="tab_wrap">
        <van-tabs swipeable v-model="selectedIndex" >
          <!-- <van-pull-refresh v-model="refresh" @refresh="selectedIndex==-1?onRefresh():getNewsList(tagList[selectedIndex])"> -->
          <van-pull-refresh v-model="refresh" @refresh="getNewsList(selectedTag)">
            <!-- 置顶新闻列表 -->
            <van-tab v-for="(item,index1) in tagList" :key="index1" :title="item">
        ...
        <!-- <div class="tab_more" @click="havePermission?showMore():goNews1()"> -->
        <div class="tab_more" @click="havePermission?showMore():goNews1()">
          <img src="../../assets/img/pub/pub_ico_zxdh.png" alt="" width="33px" height="33px">
        </div>
        <div class="more_wrap" v-show="isShowMore">
          <div class="more_item flex_c" @click="goNews1()">
            <div><span>更多资讯</span></div>
          </div>
    ...
    <!-- <van-popup v-model="showSortTag" position="bottom" style="height: 100%;"> -->
      ...
      <tagSetting :showSortTag="showSortTag" typeId='3' @closed="getTag" :dwId='orgId'></tagSetting>
    <!-- </van-popup> -->

    <van-popup v-model="longtapFlag">
      <div v-show="manager">
        <template v-if="!topFlag">
          <van-button size="small" style="width: 100%;" @click="setNewsTop(true)">新闻置顶</van-button>
        </template>
        <template v-else>
          <van-button size="small" style="width: 100%;" @click="setNewsTop(false)">取消置顶</van-button>
        </template>
        <van-button size="small" style="width: 100%;" @click="deleteNews(longTapNewsInfo)">屏蔽新闻</van-button>
        ...
        <van-button size="small" style="width: 100%;" @click="updateTitleFlag = true">修改标题</van-button>
      </div>
...
<script>
  ...
  import {
    deleteNews2,addTagOnNews,newsAdd,collectSQ,getDwNewsTagList,getDwTagList,setTop
  } from "../api/newsApi"
  import {
    getUserTagList,
    deleteUserTag,
  ...
  export default {
    ...
    methods: {
      ...
      onRefresh(selectedTag) {
        ...
        //刷新 置顶新闻列表 ， 普通新闻列表
        that.newsTop(that.orgId,selectedTag);
      ...
      loadRelease() {
        ...
        let lock = true
        that.getNewsList(this.selectedTag,lock);
        console.log(that.page,'上拉加载page2')
      },
      newsTop(orgId,selectedTag) { //把tagId 改为了selectedTag
        let that = this;
        ...
        getDwNwsList(that.page, params, (res) => {
          ...
          if (res.success) {
            ...
            if(lock){
              ...
              that.newsList = that.formatJson(list);
            }
    ...
    activated() {
      ...
      //   }
      // })
      getDwTagList(that.orgId,3,res=>{
        console.log(res,'kkkkkkkk')
        this.dwNewsTagList = res.data
        that.onRefresh(this.tagList[0])
      })
    },
    props: {
      orgId: {
    ...
    watch: {
      selectedIndex(val){
        this.selectedTag = this.tagList[val]
        this.getNewsList(this.tagList[val])
      }
    },
    computed: {
      computed_list() {
