<template>
  <div class="sl_container shopList">
    <!-- :style="!hideHeadAndTagList?`padding-top: 45px;`:''" -->
    <div v-if="havePermission" @click="pageTo(`/addactivity/${dwId}`)" style="width:1rem;height:1rem;position: fixed;bottom: 100px;right: 30px;z-index: 2;">
    ...
    <!-- <ZjHead left-text="社区资讯" @click-left="pageBack()" :fixed= true :isPageHead= true></ZjHead> -->
    <div class="tab_wrap">
      <van-tabs swipeable v-model="selectedIndex" >
        <GetList ref="load" @getMore="$refs.load.onLoadDone(true)" @refresh="selectActivityList(dwId,selectedvalue)">
          <van-tab v-for="(item1,index) in tagList" :key="index" :title="item1.name">
            <!-- 主体 -->
            <section>
              <div class="sl_shop_list" v-show="showData" style="margin-top: 44px;padding:0.2rem">
      ...
      <div class="more_wrap" v-show="isShowMore">
        <!-- <van-popup v-model="isShowMore" position="bottom"> -->
    ...
    <!-- <van-popup v-model="showSortTag" position="bottom" style="height: 100%;"> -->
      <!-- <ZjHead left-text="标签排序(长按拖动)" @click-left="showSortTag=false" right-text="保存" @click-right="updateTagListSort()"></ZjHead> -->
      <tagSetting :showSortTag="showSortTag" typeId='0' @closed="getTag" :dwId='dwId'></tagSetting>
    <!-- </van-popup> -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDwTagList(dwId){
        getDwTagList(dwId,0,(res)=>{
          console.log(res,'aaaaaqqqqq')
          // let tagArr = []
          ...
          if(this.tagList.length>0){
            ...
            if(!flag){
              this.selectActivityList(this.dwId,this.selectedvalue)
            }
          }else{
    ...
    watch:{
      ...
      // },
      selectedIndex(nv){
        this.selectedvalue = this.tagList[nv]
        this.selectActivityList(this.dwId,this.selectedvalue)
      }
    },
    activated() {
    ...
    computed: {
      computed_activityList() {
        ...
        // }
        if (this.selectedvalue.name != '全部') {
          array = this.activityList.filter((item) => {
            return item.userTagList.indexOf(this.selectedvalue.name) != -1
          })
        }else{
          array = this.activityList
        }
