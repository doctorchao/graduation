  ...
  <div class="sl_container" id="scrollDiv">
    ...
    <div class="buttomDiv" :style="checkWxDev() ? 'bottom:40px' : isIos?'bottom:70px':''  " @click="fileBtnFlag = false">
      <div class="bottomBtnDiv">
        ...
        <template v-else>
          <van-row>
            <van-col span="3" style="border-right: 0.5px solid #D2D2D2;">
...
<script>
  ...
  import { getRoleById } from '../api/detailApi'
  ...
  export default {
    ...
    activated() {
      ...
      var val1 = localStorage.getItem('realShow')
      if(val1){
          that.isHide = false
      }
      if(that.personal == 1){
        ...
        getRoleById(item,(res)=>{
          ...
          that.pageTo(`/orgModuleList/org/${teamId}`,true)
        })
      }
      that.orgId = "";
      that.groupId = "";
...
<style scoped="scoped">
  ...
  .stripe {
    font-size: 9px;
    padding: 3px;
    background-color: rgba(179, 210, 241, 0.7);
  }
